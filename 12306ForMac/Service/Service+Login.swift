...
// MARK: - Request Flow
    ...
    func loginFlow(user:String,passWord:String,randCodeStr:String,success:@escaping ()->(),failure:@escaping (_ error:NSError)->()){
        after(interval: 1).then(execute: { () -> Promise<Void> in
...
// MARK: - Chainable Request
    func loginInit()->Promise<String>{
        return Promise{ fulfill, reject in
            ...
            Service.Manager.request(url, method: .get, headers:headers).responseString(completionHandler:{response in
    ...
    func getPassCodeNewForLogin()->Promise<NSImage>{
        return Promise{ fulfill, reject in
            ...
            Service.Manager.request(url, method: .get, headers:headers).responseData(completionHandler:{response in
                switch (response.result){
                    case .failure(let error):
                        reject(error)
