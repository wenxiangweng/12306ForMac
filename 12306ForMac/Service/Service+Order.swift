...
// MARK: - Request Flow
    func submitFlow(success:@escaping () -> (),failure:@escaping (_ error:NSError)->()){
...
// MARK: - Chainable Request
    ...
    func queryOrderWaitTime(_ failMethod:@escaping (_ error:NSError)->(), waitMethod :@escaping (_ info:String) -> (),finishMethod:@escaping ()->()) {
        ...
        Service.Manager.request(url + params, method: .post, headers:headers).responseJSON(completionHandler:{response in
            switch (response.result){
            case .failure(let error):
                logger.error(error.localizedDescription)
